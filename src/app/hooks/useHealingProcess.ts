...
export function useHealingProcess({
  ...
  const healRepo = async () => {
    ...
    const aiModel = groqKey ? 'llama-3.3-70b (groq)' : gcloudKey ? 'gemini-2.0-flash (grounded)' : 'gemini-2.0-flash';
    ...
    if (project.owner && project.repoName && effectivePat) {
      ...
      try {
        ...
        await delay(600); if (!healingRef.current) return;
        ...
        if (isGitLab) {
          ...
          await delay(400); if (!healingRef.current) return;
          ...
          const glEvent = await apiCreateEvent({ pipeline_id: failedPipeline.id, project_name: `${owner}/${repoName}`, branch: failedPipeline.ref, provider: 'gitlab', status: 'healing' });
          await delay(500); if (!healingRef.current) return;
          ...
          if (ciFiles.length === 0) {
            ...
            if (glEvent) await apiUpdateEvent(glEvent.id, { status: 'failed', root_cause: 'no_ci_config_found' });
          ...
          await delay(300); if (!healingRef.current) return;
          ...
          if (aiKey) {
            ...
            const glContext = await buildGitlabContext(effectivePat, owner, repoName, branch, ciFiles, glMultiDiag.all);
              ...
              if (result && result.fixes.length > 0) {
                ...
                if (aiConfidence < APPROVAL_THRESHOLD) {
                  ...
                  if (!approved) {
                    ...
                    if (glEvent) await apiUpdateEvent(glEvent.id, { status: 'failed', root_cause: aiAnalysis || 'operator_rejected_low_confidence_fix', confidence: aiConfidence });
                    onEventUpdated?.();
                    setSystemStatus('stopped');
                    onHealingComplete?.(project.id, 'HIGH');
                    healingRef.current = false;
          ...
          if (glFixes.length === 0) {
            ...
            if (glEvent) await apiUpdateEvent(glEvent.id, { status: 'failed', root_cause: 'no_fixes_generated' });
            onEventUpdated?.();
            setSystemStatus('stopped');
            onHealingComplete?.(project.id, 'HIGH');
            healingRef.current = false;
          ...
          if (safeMode) {
            ...
            if (glEvent) await apiUpdateEvent(glEvent.id, { status: 'failed', root_cause: `[SAFE_MODE] ${aiAnalysis || 'analysis_only'}`, confidence: aiConfidence });
            onEventUpdated?.();
            setSystemStatus('stopped');
          ...
          if (!glBranchOk) {
            ...
            if (glEvent) await apiUpdateEvent(glEvent.id, { status: 'failed', root_cause: 'branch_creation_failed' });
            onEventUpdated?.();
            setSystemStatus('stopped');
            onHealingComplete?.(project.id, 'HIGH');
            healingRef.current = false;
          ...
          if (glCommitted === 0) {
            ...
            if (glEvent) await apiUpdateEvent(glEvent.id, { status: 'failed', root_cause: 'all_commits_failed' });
            onEventUpdated?.();
            setSystemStatus('stopped');
            onHealingComplete?.(project.id, 'HIGH');
            healingRef.current = false;
          ...
          if (mrUrl) {
            ...
            if (glEvent) await apiUpdateEvent(glEvent.id, {
              status: 'healed',
            ...
            if (glCiVerify === 'success') {
              ...
              if (aiKey) {
                try {
                  await delay(6_000); if (!healingRef.current) return;
                  ...
                  if (deepPipeline) {
                    ...
                    const deepContext = await buildGitlabContext(effectivePat, owner, repoName, glFixBranch, deepCiFiles, deepDiag.all);
            ...
            if (glEvent) await apiUpdateEvent(glEvent.id, { status: 'failed', root_cause: 'mr_creation_failed' });
            onEventUpdated?.();
            setSystemStatus('stopped');
            onHealingComplete?.(project.id, 'HIGH');
          }
          ...
          const ghEvent = await apiCreateEvent({ pipeline_id: failedRun.id, project_name: `${owner}/${repoName}`, branch, provider: 'github', failed_stage: allFailedRuns.map(r => r.name).join(', '), status: 'healing' });
          await delay(500); if (!healingRef.current) return;
          ...
          if (workflowFiles.length === 0) {
            ...
            add({ type: 'decision', message: 'HALT_EXECUTION :: manual_intervention_required', timestamp: ts() });
            if (ghEvent) await apiUpdateEvent(ghEvent.id, { status: 'failed', root_cause: 'no_workflow_files' });
            setSystemStatus('stopped');
            onHealingComplete?.(project.id, 'HIGH');
            healingRef.current = false;
            setActiveNode(undefined);
          ...
          let ghContextFiles: Array<{ path: string; content: string }> = [];
          let aiConfidence = 75;
          let aiAnalysis = '';
          let aiRankedAlternatives: Array<{ description: string; confidence: number; risk: string }> = [];
          let aiSources: Array<{ title: string; url: string }> = [];
          ...
          if (aiKey) {
            ...
            const allGhJobLogs = await Promise.all(
              allFailedJobs.slice(0, 5).map(async j => {
                if (!j.id) return '';
                const raw = await getJobLogs(githubPat, owner, repoName, j.id).catch(() => '');
                return `\nJob "${j.name}" logs:\n${chunkLogs(raw)}`;
              }),
            );
            ...
            const ghContext = await buildGithubContext(effectivePat, owner, repoName, branch, workflowFiles, ghMultiDiag.all);
            ...
            if (ghRuleFixes.length > 0 && !ghAllUnknown) {
              ...
              ghFixes = ghRuleFixes.map(f => {
                const orig = ghContext.allFiles.find(w => w.path === f.path)
                  ?? ghContext.allFiles.find(w => w.path.endsWith(f.path) || f.path.endsWith(w.path.split('/').pop()!));
                return { ...f, sha: orig?.sha ?? '' };
              });
            } else {
              ...
              if (result && result.fixes.length > 0) {
                ...
                ghFixes = result.fixes.map(f => {
                  const orig = ghContext.allFiles.find(w => w.path === f.path)
                    ?? ghContext.allFiles.find(w =>
                      w.path.endsWith(f.path) || f.path.endsWith(w.path.split('/').pop()!),
                    );
                  return { ...f, sha: orig?.sha ?? '' };
                });
                ...
                if (aiConfidence < APPROVAL_THRESHOLD) {
                  ...
                  if (!approved) {
                    add({ type: 'decision', message: 'HEALING_CANCELLED :: operator_rejected_low_confidence_fix', timestamp: ts() });
                    if (ghEvent) await apiUpdateEvent(ghEvent.id, { status: 'failed', root_cause: 'operator_rejected_low_confidence_fix', confidence: aiConfidence });
                    onEventUpdated?.();
                    setSystemStatus('stopped');
                    onHealingComplete?.(project.id, 'HIGH');
                    healingRef.current = false;
          ...
          if (staticFixes.length > 0) {
            ...
            const allKnownFiles = ghContextFiles.length > 0 ? ghContextFiles : (workflowFiles as Array<{ path: string; content: string; sha?: string }>);
            const staticWithSha: FixWithSha[] = staticFixes.map(f => {
              const orig = allKnownFiles.find(
                w => w.path === f.path ||
                  w.path.endsWith(f.path) ||
                  f.path.endsWith(w.path.split('/').pop()!)
              );
              return { ...f, sha: (orig as { sha?: string })?.sha ?? '' };
            });
            if (ghFixes.length === 0) {
          ...
          if (ghFixes.length === 0) {
            add({ type: 'decision', message: 'HALT_EXECUTION :: no_fixes_available manual_intervention_required', timestamp: ts() });
            if (ghEvent) await apiUpdateEvent(ghEvent.id, { status: 'failed', root_cause: 'no_fixes_generated' });
            onEventUpdated?.();
            setSystemStatus('stopped');
            onHealingComplete?.(project.id, 'HIGH');
            healingRef.current = false;
          ...
          if (safeMode) {
            ...
            if (ghEvent) await apiUpdateEvent(ghEvent.id, { status: 'failed', root_cause: `[SAFE_MODE] ${aiAnalysis || 'analysis_only'}`, confidence: aiConfidence });
            onEventUpdated?.();
            setSystemStatus('stopped');
            onHealingComplete?.(project.id, confidenceToLevel(aiConfidence, false));
            healingRef.current = false;
            ...
            if (prBranchStatus === 'failure') {
              ...
              try {
                ...
                if (resumeRuns.length > 0) {
                  ...
                  const resumeContext = await buildGithubContext(effectivePat, owner, repoName, existingAegisPR.branch, resumeWorkflows, resumeDiag.all);
                  ...
                  if (uniqueResumeFixes.length > 0) {
                    add({ type: 'result', message: `RESUME_FIX :: found ${uniqueResumeFixes.length} additional fix(es) for existing PR branch`, status: 'success', timestamp: ts() });
                    for (const fix of uniqueResumeFixes) {
                      const existingSha = resumeContext.allFiles.find(w => w.path === fix.path)?.sha ?? '';
                      await commitFile(effectivePat, owner, repoName, fix.path, fix.content, existingSha,
                        `fix(aegis-resume): ${fix.explanation.substring(0, 60)}`, existingAegisPR.branch);
                      await delay(300); if (!healingRef.current) return;
                      add({ type: 'result', message: `fix_committed :: path=${fix.path} → ${existingAegisPR.branch}`, status: 'success', timestamp: ts() });
                    }
                    add({ type: 'info', message: `RESUME_COMPLETE :: committed ${uniqueResumeFixes.length} fix(es) to existing PR — PR: ${existingAegisPR.html_url}`, timestamp: ts() });
                  } else {
          ...
          if (!branchOk) {
            add({ type: 'result', message: `execution_failed :: reason=branch_creation_failed check_token_permissions`, status: 'failure', timestamp: ts() });
            if (ghEvent) await apiUpdateEvent(ghEvent.id, { status: 'failed', root_cause: 'branch_creation_failed' });
            onEventUpdated?.();
            setSystemStatus('stopped');
            onHealingComplete?.(project.id, 'HIGH');
            healingRef.current = false;
          ...
          if (ghCommitted === 0) {
            add({ type: 'decision', message: 'HALT_EXECUTION :: all_commits_failed', timestamp: ts() });
            if (ghEvent) await apiUpdateEvent(ghEvent.id, { status: 'failed', root_cause: 'all_commits_failed' });
            onEventUpdated?.();
            setSystemStatus('stopped');
            onHealingComplete?.(project.id, 'HIGH');
            healingRef.current = false;
          ...
          if (prUrl) {
            ...
            if (ghEvent) await apiUpdateEvent(ghEvent.id, {
              status: 'healed',
              root_cause: aiAnalysis,
            ...
            if (ghCiVerify === 'success') {
              ...
              if (ghCiVerify === 'timeout') {
                ...
                if (aiKey) {
                  await delay(45_000); if (!healingRef.current) return;
              ...
              if (aiKey) {
                try {
                  ...
                  if (deepRuns.length > 0) {
                    ...
                    const deepContext = await buildGithubContext(effectivePat, owner, repoName, fixBranch, deepWorkflows, deepDiag.all);
                    ...
                    if (deepRuleFixes.length > 0) {
                      add({ type: 'result', message: `DEEP_FIX :: rule_match=${deepRuleFixes.length} patterns categories=[${deepCategories.slice(0, 3).join(', ')}]`, status: 'success', timestamp: ts() });
                      deepFixes = deepRuleFixes.map(f => ({
                        ...f, sha: deepContext.allFiles.find(w => w.path === f.path)?.sha ?? '',
                      }));
                    } else {
                      const deepErrCtx = sanitizeForAI([
                        `SECOND-PASS: CI still red on fix branch after first fix was applied.`,
                      ...
                      if (deepResult && deepResult.fixes.length > 0) {
                        const short = deepResult.analysis.length > 80 ? deepResult.analysis.substring(0, 80) + '…' : deepResult.analysis;
                        add({ type: 'result', message: `DEEP_FIX :: ai conf=${deepResult.confidence}% :: ${short}`, status: 'success', timestamp: ts() });
                        deepFixes = deepResult.fixes.map(f => ({
                          ...f, sha: deepContext.allFiles.find(w => w.path === f.path)?.sha ?? '',
                        }));
                      }
                    }
            ...
            if (ghEvent) await apiUpdateEvent(ghEvent.id, { status: 'failed', root_cause: 'pr_creation_failed' });
            onEventUpdated?.();
            setSystemStatus('stopped');
            onHealingComplete?.(project.id, 'HIGH');
          }
        ...
        if (healingRef.current) {
          add({ type: 'decision', message: `ERROR :: ${err instanceof Error ? err.message : 'unknown_error'}`, timestamp: ts() });
          setSystemStatus('stopped');
          onHealingComplete?.(project.id, 'HIGH');
        }
      }
    ...
    // ── DEMO MODE ────────────────────────────────────────────────────────────────
    const demoSequence: Array<{ delay: number; node?: string; entry?: Omit<StreamEntry, 'id'>; status?: SystemStatus }> = [
      { delay: 500,  node: 'code-push',      entry: { type: 'info',     message: 'git_push_event :: branch=main commit=a3f4b2c', timestamp: ts() } },
      { delay: 1500, node: 'pipeline',       entry: { type: 'detected', message: 'ci_pipeline_failure :: error=ENV_MISSING key=API_KEY', timestamp: ts() } },
      { delay: 2500, node: 'error-detection',entry: { type: 'attempt',  message: '#1 :: strategy=inject_env_var target=.github/workflows', timestamp: ts() } },
      { delay: 3500, node: 'error-detection',entry: { type: 'result',   message: 'execution_failed :: reason=var_not_persisted exit_code=1', status: 'failure', timestamp: ts() } },
      { delay: 4500, node: 'fix-engine',     entry: { type: 'attempt',  message: '#2 :: strategy=ai_analysis model=gemini-1.5-flash confidence=72%', timestamp: ts() } },
      { delay: 5500, node: 'fix-engine',     entry: { type: 'result',   message: 'ai_analysis_complete :: confidence=72% :: missing env var in workflow', status: 'success', timestamp: ts() } },
      { delay: 6200, node: 'fix-engine',     entry: { type: 'info',     message: 'RANKED_ALTERNATIVES :: "pin runner to ubuntu-22.04" conf=55% risk=low | "add fallback env" conf=40% risk=medium', timestamp: ts() } },
      { delay: 7000, node: 'fix-engine',     entry: { type: 'attempt',  message: '#3 :: strategy=commit_fix path=.github/workflows/ci.yml', timestamp: ts() } },
      { delay: 8000,                         entry: { type: 'decision', message: 'HALT_EXECUTION :: manual_intervention_required severity=critical', timestamp: ts() } },
      { delay: 8500, status: 'stopped' },
    ];

    demoSequence.forEach(({ delay: d, node, entry, status }) => {
      setTimeout(() => {
        if (!healingRef.current) return;
        if (node) setActiveNode(node);
        if (entry) add(entry);
        if (status) { setSystemStatus(status); healingRef.current = false; setActiveNode(undefined); }
      }, d);
    });
  };
